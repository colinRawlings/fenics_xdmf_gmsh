...
@pytest.mark.parametrize(
    "script", glob.glob(os.path.join(EXAMPLE_SCRIPTS_DIR, "parallel*.py"))
)
def test_parallel_execution_with_mpi(script):

    NUM_THREADS = 2

    parallel_calc_script_filepath = os.path.join(
        os.path.dirname(__file__), os.pardir, "examples", script
    )
