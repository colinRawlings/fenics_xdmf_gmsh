...
Tdeg = 21
lD_nm = ph.n0_M_to_lD_nm(0.01,Tdeg) # Debye length
...
for p in range(len(Lr_nm)):
    ...
    if ier==1:    
        ...
        phi_d,Ff,Fch,u_,mesh = calc_phi_d_PB(x_ND,sigma_NDr[p,:],full_output=True,doPlots=False)
        Fchr[p] = Fch        
        Ffr[p] = Ff
    else:
        print('failed')
...
for p in range(len(Lr_nm)):
    ...
    axUP[0].plot(xc_lD,sci(xc)/surf_ND['Gamma'],'-',\
        color=fh.chot(p,len(Lr_nm)),label='L: %.1f$\lambda_D$'%(Lr_nm[p]))
#
...
#======== save
fName_save = folderResults+r'Manciu_bench.pickle'

params_ND['sigma_Si']=float('nan') # can't pickle a dolfin expression

R = dict()
for fld in ['geom_ND','params_ND','geom_nm','lD_nm','pH_beh','sigma_beh',\
            'sigma_NDr','surf_ND']:
    R[fld]=eval(fld)
