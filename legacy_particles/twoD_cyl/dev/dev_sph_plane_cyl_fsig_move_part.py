...
Rs_mesh = 0.15 # cell size as a function of 1/k0 at the charge sheets
...
a_nm = 30
Lr_nm = np.linspace(5*lD_nm+2*a_small,2*R_h_min*lD_nm+lD_nm+2*a_small,NL)
...
def calc_phi_d_PB(x_ND,sigma_ND,full_output=False,doPlots=False):
    ''' 
    ...
    #params_ND['sigma_poly'] = avg(pyExpression(sci))
    
    #--- get phi_d
    Fr_int,Fch_int,Fc_int,u_,mesh = ph.solve_ratchet_problem(geom_ND,params_ND,SDs,fName_tail,\
        doPlots=doPlots,cylSim=True,disp_log=False)
...
for p in range(len(Lr_nm)):
    ...
    hr_nm.append(np.linspace(Lr_nm[p]-2*geom_nm['a']-h_min,h_min,Nh)) # start from a large separation where s0 sigma_ND_inf should be a good guess
    ...
    for q in range(Nh):
        ...
        if ier==1:    #success
            ...
            phi_d,Ff,Fch,u_,mesh = calc_phi_d_PB(x_ND,sigma_NDr[p,:],full_output=True,doPlots=(p==0 and q==0))
            Fchr[p,q] = Fch        
            Ffr[p,q] = Ff
        else:
            print('failed')
