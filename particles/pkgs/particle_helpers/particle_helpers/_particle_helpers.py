...
def calc_sph_surf_pot(params, R_balloon=4, R_mesh=0.05, doPlot=False):
    ...
    F = r ** 2 * fn.Constant(params['sigma'] / params['er']) * v * ds(1) - \
        ...
        2 * fn.Constant(params['n0'] / params['er']) * r ** 2 * ufl.sinh(u) * v * fn.dx
...
def conv_to_ratchet_ND_pars(geom_nm, lD_nm, er_dry, Tdeg, psi_mV):
    ...
    geom_ND = {}
    for par, val_nm in six.iteritems(geom_nm):
...
def calc_cylSym_ratchet_problem(geom_ND, params_ND, SDs, fName_tail, doPlots=True):
    ...
    lbled_mesh = fu.convert_2d_gmsh_geo_to_fenics_mesh(fName_tail, geom_ND, do_plots=doPlots)
    
    V = fn.FunctionSpace(lbled_mesh.mesh, 'CG', 2)
    u_ = fn.Function(V)
    u = fn.TrialFunction(V)
    v = fn.TestFunction(V)

    # -- define equation
    ds = fn.Measure('dS')[lbled_mesh.boundary_mesh_func] 
    dx = fn.Measure('dx')[lbled_mesh.subdomain_mesh_func]
    dx_wet = dx(SDs['vol']['fluid'])
    vol_dry = ['sph', 'poly', 'Si']

    r, z = fn.SpatialCoordinate(lbled_mesh.mesh)

    # -- fluid terms
    F = v / (r + 1e-6) * fn.Dx(u, 0) * params_ND['er_fluid'] * dx_wet \
        - 2 * params_ND['n0'] * ufl.sinh(u) * v * dx_wet \
    ...
    return problem, u_, lbled_mesh.mesh, lbled_mesh.subdomain_mesh_func, lbled_mesh.boundary_mesh_func
...
def desc_part_sim(Rl, dgap_nm, lD_nm):
    ...
    if 'dgapr_nm' in list(Rl['dim_vars'].keys()):
        desc['sim_type'] = 'gap size'
    elif 'lDr_nm' in list(Rl['dim_vars'].keys()):
    ...
    for key, val in six.iteritems(Rl['dim_vars']['psi_mV']):
    ...
    str = '';
    for key, val in six.iteritems(fld_desc):
