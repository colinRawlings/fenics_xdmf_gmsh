...
fName_tail = os.path.join(working_directory, "mesh", "cyl_sph_plane_tail_parametric.geo")
fName_log = os.path.join(working_directory, "logFile.txt")
...
const = {"e": 1.602e-19, "e0": 8.85e-12, "kB": 1.38e-23, "Na": 6.022e23}

psi_mV = {
    "Si": 1e3 * psi_p_ND / const["e"] * const["kB"] * (Tdeg + 273),
    "sph": 1e3 * psi_s_ND / const["e"] * const["kB"] * (Tdeg + 273),
    "poly": 1e-3,
}
geom_nm = {
    "w": w,
...
# --- calc hr
hr_nm = np.linspace(0.5 * lD_nm, 5 * lD_nm, Nh)
...
for p in range(len(hr_nm)):
    ...
    Fr_int[p], Fch, Fc_int, u_, mesh = ph.solve_ratchet_problem(
        geom_ND, params_ND, SDs, fName_tail, doPlots=(p == 0), cylSim=True
    )
...
f = (
    100.0
    / 94
    * np.asarray([24, 21, 18, 15.5, 13.5, 12, 11, 10, 8.5, 7.5, 7.0, 6, 6, 5.5, 5.0, 4])
)
N = 100
...
# --- do XX
f0, axs0 = plt.subplots(nrows=1, ncols=1)

fh.plotXX(
    u_,
    (0, -geom_ND["sb"]),
    (0, geom_ND["L"] + geom_ND["st"]),
    IV="x1",
    axs=axs0,
    style="-k",
)
axs0.set_xlabel("$z/l_0$")
axs0.set_ylabel("$e\phi/k_B T$")

f1, axs1 = plt.subplots(nrows=1, ncols=1)

fh.plotXX(
    u_,
    (0, geom_ND["L"] - geom_ND["h"] - geom_ND["a"]),
    (geom_ND["w"], geom_ND["L"] - geom_ND["h"] - geom_ND["a"]),
    IV="x0",
    axs=axs1,
    style="-k",
)
axs1.set_xlabel("$r/l_0$")
axs1.set_ylabel("$e\phi/k_B T$")

# -- add in F energy curve
f, axs2 = plt.subplots(nrows=1, ncols=1)

axs2.plot(
    Hrc,
    Fi_kT + (np.interp(2 * lD_nm, hr_nm, Fr_int) - np.min(Fr_int)),
    "-r",
    label="Stankovich96",
)
axs2.plot(hr_nm / lD_nm, Fr_int - np.min(Fr_int), ".b", label="fenics")
axs2.legend(loc="best")
axs2.set_xlabel("$x/\lambda_D$")
axs2.set_ylabel("$F/(k_BT)$")
axs2.set_title(
    "$a=%.1f\lambda_D$, $e\phi/k_BT$: sph=%.0f, pl=%.0f" % (a_ND, psi_s_ND, psi_p_ND)
)
